...
w | sed '1,2d' | awk '{print "Пользователь: " $1, "| IDLE-время: " $5, "| Процессы: " $8}'
echo

# История входов текущего пользователя
CURRENT_USER=$(whoami)
echo "История входов для текущего пользователя: $CURRENT_USER"
last $CURRENT_USER | head -n -2 | awk '{print "Время входа: " $4, $5, $6, $7, "| Время выхода: " $10, "| Длительность: " $9, "| IP-адрес: " $3}'
echo
